...
class RandomAgent(object):
    ...
    def __init__(self, env, opt):
        ...
        self.nbEvents=0
        self.gamma = 0.9
        self.qnetwork_local = NN(env.observation_space.shape[0], env.action_space.n, layers=[200])
    ...
    def learn(self):
        ...
        if self.test:
            return 
        idx, w, batch = self.buffer.sample(100)
        states = torch.FloatTensor([x[0] for x in batch]) 
        actions = torch.FloatTensor([x[1] for x in batch]).type(torch.int64).resize_((100,1))
        rewards = torch.FloatTensor([x[2] for x in batch])
        next_states = torch.FloatTensor([x[3] for x in batch])
        dones = torch.FloatTensor([x[4] for x in batch])

        predicted_targets = self.qnetwork_local(states).squeeze().gather(1, actions).squeeze()
        labels_next = self.qnetwork_target(next_states).squeeze().max(1)[0].detach()
