...
class RandomAgent(object):
    ...
    def __init__(self, env, opt, gae=False, gridworld=False):
        self.opt=opt
        self.env=env
        self.gae=gae
        ...
        self.optimizer = optim.Adam(self.critic.parameters(), lr=1e-4) # best values so far: 8e-3 with 5e-3
    ...
    def learn(self):
        ...
        next_value = self.critic_target(torch.FloatTensor(self.new_states[-1])).item()
        ...
        for t in reversed(range(len(self.rewards))):
            if t == len(self.rewards)-1:
                returns[t] = self.rewards[t] + self.gamma * (1-self.dones[t]) * next_value
                delta = self.rewards[t] + self.gamma * (1-self.dones[t]) * next_value - self.values[t].item()
            else:
        ...
        states = torch.stack(self.states, dim=0).squeeze().detach()
        actions = torch.stack(self.actions, dim=0).squeeze().detach()
        log_probs = torch.stack(self.log_probs, dim=0).squeeze().detach()

        value = self.critic(states).view(-1)
        old_dist = self.actor(states).view((-1, self.action_space.n))
        ...
        for _ in range(self.K):
            ...
            actor_loss  = -(curr_log_prob * gae).mean()
            self.actor_count += 1
        ...
        new_dist = self.actor(states).view((-1, self.action_space.n))
        kl_div = F.kl_div(new_dist, old_dist.view((-1, self.action_space.n)), reduction='batchmean')
        self.kl_loss = kl_div

        critic_loss = F.huber_loss(value, target)
        ...
        logger.direct_write('Actor Loss', actor_loss, self.t_step)
        logger.direct_write('Critic Loss', critic_loss, self.t_step)
        logger.direct_write('Q value', q_moy, self.t_step)

        if self.t_step%self.C == 0:
            for target_param, local_param in zip(self.critic_target.parameters(), self.critic.parameters()):
                target_param.data.copy_(local_param.data)

        return self, actor_loss, critic_loss
